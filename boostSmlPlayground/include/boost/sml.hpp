...
const char* get_type_name()
{
    ...
    return detail::get_type_name<T, 58>(__PRETTY_FUNCTION__,
...
struct transitions<T>
{
    ...
    static bool execute_impl(const on_exit<_, TEvent>& event,
                             SM& sm,
                             TDeps& deps,
                             TSubs& subs,
...
struct sm_impl
{
    ...
    using events_t =
        aux::apply_t<aux::unique_t, aux::join_t<sub_internal_events_t, aux::apply_t<get_events, transitions_t> > >;
...
#else
        const auto handled = process_event_noexcept<get_event_mapping_t<get_generic_t<TEvent>, mappings> >(
    ...
    void initialize(const aux::type_list<TStates...>&)
    {
        ...
        (void)aux::swallow{
            0, (region = i, current_state_[region] = aux::get_id<state_t, TStates>((states_ids_t*)0), ++i, 0)...};
    }
                            ...
                            aux::index_sequence<0>)
    {
        ...
        const static dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TStates))] = {
            &get_state_mapping_t<TStates,
                                 TMappings,
                                 has_unexpected_events>::template execute<TEvent, sm_impl, TDeps, TSubs>...};
                            ...
                            aux::index_sequence<Ns...>)
    {
        ...
        (void)lock;
        (void)aux::swallow{
            0, (handled |= dispatch_table[current_state_[Ns]](event, *this, deps, subs, current_state_[Ns]), 0)...};
        return handled;
    }
    template <class TMappings, class TEvent, class TDeps, class TSubs, class... TStates>
    bool process_event_impl(const TEvent& event,
    ...
    void process_defer_events(TDeps&,
                              TSubs&,
                              const bool,
    ...
    bool process_event_no_defer(TDeps& deps, TSubs& subs, const void* data)
    {
        ...
        log_process_event<sm_t>(aux::type<logger_t>{}, deps, event);
#if BOOST_SML_DISABLE_EXCEPTIONS
        const auto handled = process_event_impl<get_event_mapping_t<TEvent, mappings> >(
            event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
        const auto handled =
            process_event_noexcept<get_event_mapping_t<TEvent, mappings> >(event, deps, subs, has_exceptions{});
#endif
        if (handled)
        {
            defer_.pop();
    ...
    void process_defer_events(TDeps& deps,
                              TSubs& subs,
                              const bool handled,
    ...
    bool is_terminated_impl(aux::index_sequence<Ns...>) const
    {
        ...
        (void)aux::swallow{
            0,
            (current_state_[Ns] == aux::get_id<state_t, terminate_state>((states_ids_t*)0) ? result
                                                                                           : (result = false))...};
        return result;
    }
...
class sm
{
  ...
  private:
    using sm_all_t =
        aux::apply_t<aux::inherit, aux::join_t<aux::type_list<sm_t>, aux::apply_t<get_sm_t, state_machines> > >;
...
template <class T, class E, class>
auto args__(int)
    -> aux::function_traits_t<decltype(&T::__BOOST_SML_TEMPLATE_KEYWORD operator() < back::get_event_t<E>>)>;
template <class T, class E, class TSM>
auto args__(int)
    -> aux::function_traits_t<decltype(&T::__BOOST_SML_TEMPLATE_KEYWORD operator() < back::get_event_t<E>, TSM >)>;
template <class T, class E, class TSM>
...
void update_current_state(SM&,
                          ...
                          const TDstState&)
{
    back::log_state_change<typename SM::sm_t>(aux::type<typename SM::logger_t>{},
                                              deps,
